     ...
     if ($_SERVER["REQUEST_METHOD"] === "POST") {
        ...
        if ($nombre === "" || $apellido === "" || $correo === "") {
            ...
            try {
                ...
                $pdo->commit();
                $mensaje = "transaccion confirmada  Alumno registrado con ID: $idAlumno";
            } catch (Exception $e) {

                if ($pdo->inTransaction()) {
                    $pdo->rollBack();
                }
                $mensaje = "Ocurrió un error trancción revertida.";
        ...
        <div class="card">
            <form method="POST">
                <div class="row">
                    <div>
                        <label>Nombre</label>
                        <input type="text" name="nombre" maxlength="15" value="<?= htmlspecialchars($_POST['nombre'] ?? 'Paul') ?>">
                    </div>
                    <div>
                        <label>Apellido</label>
                        <input type="text" name="apellido" maxlength="10" value="<?=  htmlspecialchars($_POST['apellido'] ?? 'Lizarraga') ?>">
                    </div>
                    <div>
                        <label>Correo</label>
                        <input type="email" name="correo" maxlength="50" value="<?=  htmlspecialchars($_POST['correo'] ?? 'lizarraga@uas.edu.mx' ) ?>">                        
                    </div>
                    </div>
